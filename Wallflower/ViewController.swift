...
class ViewController: UIViewController {
	...
	@IBAction func addPhotoPressed(sender: AnyObject) {
		Alamofire.request(.POST, "https://services-qa.walgreens.com/api/util/mweb5url", parameters: [
			"serviceType": "wagS3",
			...
			.responseJSON { (response) -> Void in
				...
				let secretKey = json["secretKey"].stringValue
				let accessKeyId = json["accessKeyId"].stringValue
				let sessionId = json["sessionId"].stringValue
				let err = json["err"].stringValue
				
				let now = NSDate()
				let timeFormatter = NSDateFormatter()
				timeFormatter.dateFormat = "MMM dd, yyyy, HH:mm"
				
				
				let yrFormatter = NSDateFormatter()
				yrFormatter.dateFormat = "Y"
				let monthFormatter = NSDateFormatter()
				monthFormatter.dateFormat = "M"
				let dayFormatter = NSDateFormatter()
				dayFormatter.dateFormat = "dd"
				
				let authHeader = "PUT\n" + "" + "\n" + "image/jpg" + "\n" + "\(timeFormatter.stringFromDate(now))" + "\n" + "x-amz-security-token:" + sessionId + "\n/" + uploadUrl + "/" + "uploadUrl/myImage_\(monthFormatter.stringFromDate(now))_\(dayFormatter.stringFromDate(now))_\(yrFormatter.stringFromDate(now))_12154510.jpg"
				print(authHeader)
		}
	}
	
	//	func sendChosenPhoto(photoFile: UIImage) {
